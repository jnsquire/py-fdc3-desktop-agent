...
class ContextListener:
    def __init__(self, listener_uuid: ListenerUuid, instance_uuid: str, context_type: Optional[str] = None):
...
class ListenerStore:
    ...
    def __init__(self):
        ...
        self.listeners_by_instance: Dict[str, Set[str]] = {}  # instance_uuid -> set of listener_uuids

    def add_context_listener(self, listener_uuid: ListenerUuid, instance_uuid: str, context_type: Optional[str] = None) -> ContextListener:
    ...
    def add_intent_listener(self, listener_uuid: ListenerUuid, instance_uuid: str, intent: str) -> IntentListener:
    ...
    def get_context_listeners(self, context_type: Optional[str] = None) -> List[ContextListener]:
        """Get all context listeners, optionally filtered by context type."""
        listeners = list(self.context_listeners.values())
        if context_type is not None:
            listeners = [listener for listener in listeners if listener.context_type == context_type or listener.context_type is None]
        return listeners

    def get_intent_listeners(self, intent: str) -> List[IntentListener]:
        """Get all intent listeners for a specific intent."""
        return [listener for listener in self.intent_listeners.values() if listener.intent == intent]
    ...
    def get_context_listeners_for_type(self, context_type: Optional[str] = None) -> List[ContextListener]:
        if context_type:
            return [listener for listener in self.context_listeners.values() if listener.context_type == context_type or listener.context_type is None]

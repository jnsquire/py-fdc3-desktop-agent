...
This is a simple, well-documented scaffold suitable for examples and tests.
It is NOT a full production-ready client (no authentication, limited error
handling). It demonstrates the required primitives: connect, WCP handshake,
register handlers, handle forwarded intents, and send results.
"""
...
class FDC3Client:
    ...
    Args:
        agent_url: WebSocket URL of the agent (e.g., "ws://localhost:8000/ws")
        handler_id: Unique identifier for this external handler
        ping_interval: Seconds between WebSocket pings (default: 20)
    """
    ...
    async def register_handler(
        ...
        *,
        ...
        Args:
            ...
            intents: List of intent names this handler can process
            priority: Handler priority (higher = preferred)
            metadata: Optional metadata dict
            timeout: Seconds to wait for response

        Returns:
            The agent-assigned handler_uuid
    ...
    async def unregister_handler(self, handler_uuid: str, timeout: float = 5.0) -> None:
        ...
        Args:
            handler_uuid: The handler UUID returned from register_handler
            timeout: Seconds to wait for response
        """
        assert self._ws is not None
